...
function ShippingProgress({
  ...
  const libre    = total >= ENVIO_GRATIS;
  const falta    = ENVIO_GRATIS - total;
  const progreso = Math.min(100, (total / ENVIO_GRATIS) * 100);

  // Producto sugerido: el más barato que cubra lo que falta, o el más barato disponible
  const disponibles = productos
    .filter((p) => !cartIds.has(p.id) && p.stock > 0)
    .sort((a, b) => a.precio - b.precio);

  const sugerido = !libre
    ? disponibles.find((p) => p.precio >= falta) ?? disponibles[0] ?? null
    : null;
    ...
    <div className="mx-5 mt-3">
      {libre ? (
        <div className="bg-[#f0fdf4] border border-[#86efac] rounded-2xl px-4 py-3 text-center">
          <p className="font-nunito font-black text-[#166534] text-sm">
        ...
        <div className="bg-[#f9fafb] rounded-2xl px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <span className="font-nunito font-black text-[#1A1A1A] text-xs">🚚 Envío gratis</span>
            <span className="font-nunito text-[#999] text-[10px]">
              Faltan <strong className="text-[#3AAA35]">${falta.toLocaleString("es-CL")}</strong>
            </span>
          </div>
          ...
          <div className="w-full bg-[#e5e5e5] rounded-full h-2 overflow-hidden">
            <div
              className="bg-[#3AAA35] h-full rounded-full transition-all duration-500 ease-out"
              style={{ width: `${progreso}%` }}
            />
          </div>

          <div className="flex items-center justify-between mt-1.5">
            <span className="font-nunito text-[10px] text-[#bbb]">$0</span>
            <span className="font-nunito text-[10px] text-[#bbb]">$20.000</span>
          </div>

          {/* Sugerencia */}
          {sugerido && (
            <div className="mt-2.5 flex items-center gap-2 bg-white border border-[#f0f0f0] rounded-xl px-2.5 py-2">
              {sugerido.imagen && (
                <div className="w-8 h-8 flex-shrink-0 relative">
                  <Image src={sugerido.imagen} alt={sugerido.nombre} fill className="object-contain" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="font-nunito font-black text-[#1A1A1A] text-[11px] truncate">
                  + {sugerido.nombre}
                </p>
                <p className="font-nunito text-[#3AAA35] text-[10px] font-black">
                  ${sugerido.precio.toLocaleString("es-CL")}
                </p>
              </div>
              <button
                onClick={() => onAdd(sugerido)}
                className="flex-shrink-0 bg-[#F9C514] hover:bg-[#E0B010] text-[#1A1A1A] font-nunito font-black text-[10px] px-2.5 py-1 rounded-full transition-colors"
              >
                Agregar
              </button>
            </div>
